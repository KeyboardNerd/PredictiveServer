...
import LPModel
from LearningModel import getCruisePhase # cruise phase is just for demonstration
import json
import sys
BETA = False
def main():
	...
	if BETA:
		trans = LPModel.LPModelTransformation(XF,YF)
		trans.setWeight(emptyWeight)
		model = LPModel.LPModel(trans)
	else:
		trans = lm.SpecialLinearTransformationP(XF,YF)
		trans.setWeight(emptyWeight)
